...
def make_message_serializable(
    ...
    if isinstance(message, dict):
        ...
        return message.strftime(settings.datetime_format)
...
def log(message: MESSAGE, exception: BaseException = None, logger_name: str = None, level: int = None):
    """
...
def warning(message: MESSAGE, logger_name: str = None):
    ...
    Mirrors the vanilla warning logic for drop in replacement

    Args:
        message: The message to log
        logger_name: The name of the logger to use. The default is used if none is passed
    """
    warn(message, logger_name)
...
def error(
    ...
    *,
    exc_info: typing.Union[BaseException, tuple] = None
):
    """
    ...
    Args:
        ...
        exc_info: An alias for the standard exception option for errors
    """
    log(message=message, exception=exception or exc_info, logger_name=logger_name, level=logging.ERROR)
